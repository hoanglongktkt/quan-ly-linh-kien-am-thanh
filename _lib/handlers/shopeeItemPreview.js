/**
 * POST /api/shopee/products/item-preview — proxy JSON-safe sang cPanel.
 */
...
export async function handleShopeeItemPreview(req, res) {
  ...
  const target = buildCpanelTarget(backend.url, 'shopee/products/item-preview', req.query || {});
  const result = await fetchWithDiagnostics(
    ...
    target,
    {
      ...
      headers: {
        ...
        'X-Proxy-Source': 'vercel-shopee-item-preview',
      },
