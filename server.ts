...
async function fetchShopeeItemListPage(
  ...
  const itemIds = items.map((it: any) => Number(it.item_id)).filter((n: number) => Number.isFinite(n) && n > 0);
...
async function processShopeeItemsToListingRows(
  ...
  await runInShopeeBatches(items, async (item) => {
    try {
...
async function pullShopeeChannelListingsPage(
  ...
  const rowsSaved = upsertChannelListingsBatch(rows, shopId, shopName);
...
async function pullShopeeChannelListingsAllPages(
  shopId: string,
  accessToken: string,
  shopName: string
): Promise<{
  ...
  while (hasMore && pageGuard < 200) {
    ...
    parentCount += page.pageStats.rowsInPage;
    itemCount += page.pageStats.itemsInPage;
    variantItemCount += page.pageStats.variantItemCount;
    skippedItems.push(...page.skippedItems);
...
function mergeWarehouseProductsBatch(batchRows: any[]): number {
  ...
  for (const row of batchRows) {
    byId.set(row.id, mergeShopeeRowPreservingLocal(byId.get(row.id), row));
  }
...
/** Ghi DB nguyên trạng — không regroup toàn bộ file. */
function saveProducts(products: any[]): void {
  try {
    const list = Array.isArray(products) ? products : [];
    fs.mkdirSync(path.dirname(PRODUCTS_DB_PATH), { recursive: true });
    fs.writeFileSync(PRODUCTS_DB_PATH, JSON.stringify(list, null, 2), "utf-8");
  } catch (error) {
...
async function startServer() {
  ...
  // API 1: Kéo dữ liệu sản phẩm sàn từ Shopee — chỉ lưu DB thô, KHÔNG auto-link SKU.
    ...
    try {
      ...
      if (fetchAll) {
        ...
        return res.status(200).json({
          ...
          hasMore: false,
          nextOffset: null,
          savedCount: allResult.totalSaved,
          fetchedCount: allResult.parentCount,
          parentCount: allResult.parentCount,
      ...
      const isPayload = /413|payload too large/i.test(message);
      if (!res.headersSent) {
        return res.status(isRate ? 429 : isPayload ? 413 : 500).json({
          success: false,
          message: isPayload
            ? "Phản hồi quá lớn — hãy dùng phân trang (offset/nextOffset) thay vì tải toàn bộ một lần."
            : message,
          details,
          error: isPayload ? "payload_too_large" : isRate ? "shopee_rate_limit" : "exception",
        });
      }
    }
  });
  ...
  app.post("/api/shopee/products/sync", authMiddleware, async (req, res) => {
    try {
      ...
      return res.status(200).json({
        ...
        message: `Đã đồng bộ ${result.productCount} sản phẩm mẹ (${result.stats?.variantSkuCount ?? 0} phân loại) từ Shopee (${result.stats?.pageCount ?? 0} trang). Hãy tải lại danh sách sản phẩm.`,
      });
    } catch (error: unknown) {
