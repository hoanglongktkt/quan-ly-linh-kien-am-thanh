...
async function ensureTrackingBeforePrint(
  ...
  // Song song theo shop: mỗi shop lấy token 1 lần, rồi fetch tracking tối đa 4 đơn cùng lúc
  // (tránh vòng for-await tuần tự + sleep 350ms/đơn — nguyên nhân chính PDF 5 đơn mất ~3 phút).
...
async function processShopeeWebhookPayload(body: any): Promise<void> {
  try {
    ...
    // Return fallback chỉ khi thật sự có return/hoàn — KHÔNG chạy cho CANCELLED thuần
    // (buyer cancel không phải return; fallback cũ có thể ghi đè status hoặc lãng phí API).
...
async function startServer() {
  ...
  async function tryGenerateShopeeShippingDocumentOnce(shopId: string, accessToken: string, orderList: { order_sn: string; package_number?: string; tracking_number?: string }[]) {
    ...
    // Check NGAY lần đầu (không sleep trước). Interval 600ms × 12 ≈ 7s max —
    // đủ cho SPX/GHN sẵn sàng, tránh 15×1.2s ≈ 18s chết cứng trước đây.
  ...
  async function autoPrintLabelsForShopeeOrders(allOrders: any[], shopeeOrders: any[]) {
    ...
    // Chỉ chờ ngắn nếu vẫn còn đơn thiếu mã — không chờ cứng 2.5s khi đã đủ tracking.
    const stillNeedTn = candidates.some((o: any) => !orderHasPrintableTracking(o));
    if (stillNeedTn) {
      console.log(
        `[Ship Order Bulk Auto-Print] Còn đơn thiếu mã — chờ 800ms rồi quét lại (${candidates.length} đơn)...`,
      );
    ...
    for (const [shopId, groupOrders] of Object.entries(groups)) {
      ...
      // Retry đơn còn thiếu — song song, không sleep 2s × N.
      const printedSet = new Set(printedOrderSns);
      const stillMissing = ready.filter((o: any) => !printedSet.has(o.orderSn));
      if (stillMissing.length > 0) {
        await sleep(800);
        const retryResults = await mapWithConcurrency(stillMissing, 3, async (o) => {
  ...
  /** Patch Mongo-only cho các đơn đã đổi — không load/ghi full orders.json. */
  async function persistShipJobOrderPatches(changedOrders: any[]): Promise<void> {
    const list = (Array.isArray(changedOrders) ? changedOrders : []).filter(Boolean);
    if (list.length === 0) return;
    if (!isMongoReady()) {
      console.warn("[Ship Order Job] Mongo chưa sẵn sàng — bỏ qua persist patch (FE optimistic).");
      return;
    }
    try {
      await bulkUpsertOrdersToStore(list);
    } catch (err: any) {
      console.warn("[Ship Order Job] bulkUpsert patch failed:", err?.message || err);
    }
  }

  /**
   * Background ship job (lean):
   * - Scoped Mongo $in load only
   * - Promise.allSettled concurrent Shopee confirm (skipRecover)
   * - Mongo patch changed orders only — no full DB save / no recover / no PDF
   */
    ...
    try {
      ...
      for (let i = 0; i < settled.length; i++) {
        ...
        if (item.status === "fulfilled") {
          const r = item.value;
          results.push(r);
          if (r.patched) changedOrders.push(r.patched);
          if (!r.success) {
            failedOrders.push({
          ...
          if (toShip[i]) changedOrders.push(orders[toShip[i].index]);
        }
      }
  ...
  // Trả 202 ngay — validate + jobId only. Mọi I/O/Shopee chạy trong setImmediate.
  ...
  async function printDocumentHandler(req: any, res: any) {
    ...
    const waitMs = Math.min(3000, Math.max(0, Number(rawWaitMs) || 0));
    if (waitMs > 0) {
    ...
    for (const [shopId, groupOrders] of Object.entries(groups)) {
      ...
      // Retry in lẻ các đơn còn thiếu — song song (concurrency 3), không sleep 1.5s tuần tự.
      const printedSet = new Set(allPrintedSns);
      const stillMissing = readyToPrint.filter((o: any) => !printedSet.has(o.orderSn));
      if (stillMissing.length > 0) {
        // Một nhịp ngắn chung thay vì 1.5s × N đơn.
        await sleep(800);
        const retryResults = await mapWithConcurrency(stillMissing, 3, async (o) => {
